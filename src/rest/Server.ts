...
import {InsightDatasetKind, NotFoundError, InsightError} from "../controller/IInsightFacade";
...
export default class Server {
	...
	public async addDataset(req: Request, res: Response) {
		...
		try {
			...
			const result = await this.insightFacade.addDataset(req.params.id, body, kind);
	...
	public performQuery(req: Request, res: Response) {
		const filePath = "./data/roomids.json";
		this.isIdInArray(req.params.id, filePath)
			.then((ifExists) => {
				if (ifExists) {
					this.insightFacade
	...
	public async isIdInArray(id: string, filePath: string): Promise<boolean> {
		try {
			// Read the array from the file
			const idArray = await fs.promises.readFile(filePath, "utf8")
				.then((data) => JSON.parse(data))
				.catch(() => []);

			// Check if the given id is in the array
